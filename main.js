/* blissolic.com — live subscriber count + email copy */

(() => {
  "use strict";

  const CHANNEL_ID = "UC2KjRb9JQ9yAEIw8HMFUENg";
  const REFRESH_MS = 300_000; // 5min: the display only moves every 100 subs
  ...
  /* Keyless, and all send Access-Control-Allow-Origin: *.
     ...
     Order matters here in a way it wouldn't at 100K+. The pill renders one
     decimal at this size, so it moves every 100 subs and the sources visibly
     disagree: socialcounts returns 25700 (YouTube's own floored figure, so the
     pill matches the channel page exactly) while mixerno's live estimate of
     25673 would render 25.6K — a notch low. socialcounts stays first for that
     reason; mixerno is a real but slightly lossy fallback. */
  const SOURCES = [
    {
  ...
  // last known good. YouTube's floored figure, not the live estimate — this is
  ...
  /* Matches YouTube's own abbreviation: 3 significant figures, TRUNCATED.
     25,673 -> "25.6K" (not "25.7K" — YouTube floors, and rounding up would read
  ...
  /* ---------- page views ----------
     ...
     public counter. Both services below are CORS-open; abacus goes first only
     because its payload is a bare {value}.

     The hit fires once per browser SESSION, not per load, so hammering F5
     doesn't inflate it — every later load just reads. The counter is public and
     anyone who finds the URL could bump it; that's the price of having no
     backend, and it's a vanity number, not analytics. */
  const VIEW_NS = "blissolic-links";
  ...
  async function showViews() {
    ...
    let counted = false;
    try {
      counted = sessionStorage.getItem("bl:counted") === "1";
    } catch {
    ...
    for (const src of VIEW_SOURCES) {
      try {
        ...
        try {
          sessionStorage.setItem("bl:counted", "1");
        } catch {
  ...
  function animateTo(target, duration) {
    ...
    // past, so the odometer reads 0 -> 1.23K -> 12.4K -> 25.7K
    const step = (now) => {
  ...
  async function copy(text) {
    ...
    // execCommand path — needed on plain http:// and older mobile browsers
  ...
  function toast(msg) {
    ...
    toastId = setTimeout(() => toastEl.classList.remove("is-visible"), 2200);
  }

  let copiedId = null;
  mailEl?.addEventListener("click", async () => {
    const email = mailEl.dataset.email;
    if (await copy(email)) {
      mailEl.classList.add("is-copied");
      toast("Email copied to clipboard");
      clearTimeout(copiedId);
      copiedId = setTimeout(() => mailEl.classList.remove("is-copied"), 2200);
    } else {
      toast(email);
    }
  });
